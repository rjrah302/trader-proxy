...
function calcCandles(c){if(!c||c.length<5)return{count:0,color:'neutral'};const last=c.slice(-5);let green=0,red=0;for(let i=1;i<last.length;i++){if(last[i]>last[i-1])green++;else red++;}return{green,red,color:green>=3?'green':red>=3?'red':'neutral'};}
function getSignal(rsi,hist,dir,weekly,candles){const signals=[];const risks=[];if(hist>0&&dir==='expanding')signals.push('زخم MACD صاعد قوي');else if(hist>0&&dir==='contracting')signals.push('زخم MACD صاعد يضعف');else if(hist<0&&dir==='expanding')risks.push('زخم MACD هابط يتسع');else if(hist<0)risks.push('زخم MACD هابط');if(rsi<35)signals.push('RSI في منطقة تشبع بيع — فرصة');else if(rsi>70)risks.push('RSI في منطقة تشبع شراء — خطر');else if(rsi>=40&&rsi<=60)signals.push('RSI في منطقة محايدة آمنة');if(weekly==='bullish')signals.push('الاتجاه الأسبوعي صاعد');else risks.push('الاتجاه الأسبوعي هابط');if(candles.green>=3)signals.push(`${candles.green} شموع خضراء متتالية`);else if(candles.red>=3)risks.push(`${candles.red} شموع حمراء متتالية`);const score=signals.length-risks.length;let verdict,icon;if(score>=3){verdict='إشارة شراء قوية — الظروف مواتية';icon='✅';}else if(score>=1){verdict='إشارة إيجابية — يمكن الدخول بحذر';icon='⚠️';}else if(score===0){verdict='إشارات متضاربة — انتظر وضوح الاتجاه';icon='⏳';}else{verdict='إشارة سلبية — تجنب الدخول الآن';icon='❌';}return{signals,risks,verdict,icon,score};}
function getWatchAdvice(rsi,hist,weekly){const tips=[];if(rsi>60)tips.push('انتظر RSI يهبط دون 50 للدخول بأمان');if(rsi<40)tips.push('RSI منخفض — قد يكون فرصة شراء قريباً');if(hist<0)tips.push('انتظر MACD يتحول إيجابياً قبل الدخول');if(weekly==='bearish')tips.push('الأسبوعي هابط — تحلى بالصبر');if(tips.length===0)tips.push('راقب كسر مستوى المقاومة كإشارة دخول');return tips;}
...
tips.forEach(t=>{ m+=`• ${t}\n`; });
m+=`──────────────\n⏰ سأتابعه وأنبهك عند إشارة دخول أفضل`;
await tgSend(m);}
...
m+=`💰 <b>$${q.price?.toFixed(2)}</b> ${chg>=0?'📈':'📉'} ${chg>=0?'+':''}${chg?.toFixed(2)}%\n`;
m+=`──────────────\n`;
