// 🦅 RamiFalconX — Vercel Serverless Function (CommonJS)
...
function calcHist(c) {
  ...
  const arr=[];
  for(let j=35;j<=c.length;j++){
    const sl=c.slice(0,j);
    const a=calcEMA(sl,12),b=calcEMA(sl,26);
    if(a&&b) arr.push(a-b);
  }
  const sig=calcEMA(arr,9);
  return sig?(e12-e26)-sig:null;
}
...
async function getStock(sym) {
  try {
    ...
    return { quote:Array.isArray(q)?q[0]:null, closes:(h?.historical||[]).map(d=>d.close).reverse() };
  } catch(e) { return null; }
}
