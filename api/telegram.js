...
let db;
function getDB(){if(!db){if(!getApps().length){const sa=JSON.parse(Buffer.from(process.env.FIREBASE_SERVICE_ACCOUNT,'base64').toString('utf8'));initializeApp({credential:cert(sa)});}db=getFirestore();}return db;}
...
function calcLevels(c,price){if(!c||c.length<20)return{support:null,resistance:null,stop:null,days:null};const last20=c.slice(-20);const support=Math.min(...last20).toFixed(2);const resistance=Math.max(...last20).toFixed(2);const atr=Math.abs(price-parseFloat(support));const stop=(price-atr*0.5).toFixed(2);const target=(price+atr*1.5).toFixed(2);const pct=((parseFloat(target)-price)/price*100);const days=Math.max(1,Math.ceil(pct/1.5));return{support,resistance,stop,target,days};}
...
else{const data=await fbGet('watchlist');const list=data.symbols||[];if(!list.includes(s.sym)){list.push(s.sym);await fbSet('watchlist',{symbols:list});}const tips=getWatchAdvice(s.rsi,s.hist,s.weekly);let m=`👁 <b>${s.sym} أضيف للمراقبة</b>\n──────────────\n`;tips.forEach(t=>{m+=`• ${t}\n`;});m+=`──────────────\n⏰ اكتب <code>مراقبتي</code> لرؤية قائمتك`;await tgSend(m);sess[cid]={};}
res.status(200).json({ok:true});return;}
...
const levels=calcLevels(d.closes,q.price);
...
m+=`──────────────\n`;
if(levels.support)m+=`🟢 دعم: <b>$${levels.support}</b>\n`;
if(levels.resistance)m+=`🔴 مقاومة: <b>$${levels.resistance}</b>\n`;
if(levels.stop)m+=`🛑 وقف مقترح: <b>$${levels.stop}</b>\n`;
if(levels.days)m+=`⏱️ مدة الاحتفاظ: <b>${levels.days<=1?'🔥 يومي':levels.days<=3?`⚡ ${levels.days} أيام`:`📅 ${levels.days} أيام`}</b>\n`;
m+=`──────────────\n🤖 <b>التحليل:</b>\n`;
